...
class Sap {
    ...
    /**
     * I seguenti attributi contengono informazioni sullo stato della response ricevuta da axios
     * Sono settati sempre dopo la chiamata: [responseStatus, responseStatusText]
     * Sono settati solo dopo la chiamata fallita: [responseMessage, responseCode]
     */
    responseStatus = null
   ...
   /**
    ...
    /**
     * Ritorna il codice numerico della response
     * @return {Number|null}
     */
    status() {
    ...
    /**
     * Settaggio codice numerico della response
     * @return {void}
     */
    setStatus(status) {
    ...
    /**
     * Verifica se il codice di risposta é di successo
     * Ritorna null se il codice non é impostato: ovvero non é avvenuta la chimata
     * @return {Boolean|null}
     */
    isOk() {
      return this.status() === null ? null : this.status() >= 200 && this.status() < 300
    }

    /**
     * Verifica se la chiamata é avvenuta con successo
     * @return {Boolean}
     */
    successful() {
    ...
    /**
     * Verifica se la chiamata ha fallito
     * @return {Boolean}
     */
    failed() {
    ...
    /**
     * Ritorna il messgagio della response
     * Salvato in caso di errore
     * @return {String|null}
     */
    message() {
    ...
    /**
     * Ritorna una stringa messaggio di log in riferimento allo stato della chiamata
     * @return {String}
     */
    logInfo() {
      if (this.isOk() === true) {
        return `${this.status()}, ${this.responseStatusText}`
      } else if (this.isOk() === false) {
        return `${this.status()}, ${this.responseStatusText}, ${this.responseCode}, ${this.message()}`
      }
      return 'UNDEFINED: is ok, request not sent yet'
    }
    ...
    async request(config) {
        ...
        // Verifica presenza parametro $select come array
        let configName = config.method == 'get' ? 'params' : 'data';
        if (config[configName] && '$select' in config[configName] && Array.isArray(config[configName].$select)) {
            config[configName].$select = config[configName].$select.join(',');
        }
        ...
        await axios(config)
            .then( (response) => {
                ...
                // Salvataggio stato di risposta e testo
            ...
            .catch( (error) => {
                ...
                // In caso di errore si salvano i codici e messaggi relativi all'errore
